@php
use App\Models\Tool;
$categories = \App\Models\Category::with('subcategories.banners')->get();
$tool = Tool::first();
@endphp
			...
			<nav class="hs-accordion-group p-3 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
				<ul class="flex flex-col space-y-1">
					...
					<li class="hs-accordion" id="account-accordion">
						<button type="button" @class([ 'hs-accordion-toggle w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-lg rounded-lg' , 'bg-[#FF9700] text-[#e5e7eb] font-semibold dark:bg-neutral-700 dark:text-blue-400'=> $isActiveCategory,
						...
						<div id="account-accordion-child-{{ $category->id }}"
							...
							<ul id="dropdown-{{ $category->id }}" class="ps-8 pt-1 space-y-1" data-hs-accordion-always-open>
								@foreach ($category->subcategories as $sub)
								@if ($sub->banners->count())
								@php
								$isActiveSub = request()->is($category->slug . '/' . $sub->slug . '/*');
								@endphp
								<li class="hs-accordion" id="sub-accordion-{{ $sub->id }}">
									<button
										class="hs-accordion-toggle w-full text-start flex items-center justify-between py-2 px-2.5 text-sm font-semibold rounded-lg text-[#e5e7eb] hover:bg-gray-100 hover:text-[#05104F] dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:text-neutral-200
                        {{ $isActiveSub ? 'bg-[#FF9700] text-white' : '' }}"
										aria-expanded="{{ $isActiveSub ? 'true' : 'false' }}"
										aria-controls="sub-accordion-content-{{ $sub->id }}">
										{{ $sub->name }}
										<svg class="hs-accordion-active:block ms-auto hidden size-4" xmlns="http://www.w3.org/2000/svg"
									...
									<div id="sub-accordion-content-{{ $sub->id }}"
										class="hs-accordion-content w-full overflow-hidden transition-[height] duration-300 {{ $isActiveSub ? 'block' : 'hidden' }}">
										<ul class="ps-4 space-y-1 pt-1">
											@foreach ($sub->banners as $banner)
											<li>
                                    ...
                                    'title' => $banner->slug,
													...
													class="block py-1 px-2.5 text-sm rounded-lg text-[#91a0ad] hover:bg-gray-100 hover:text-[#05104F]
                                        dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:text-neutral-200
                                        {{ request()->is($category->slug . '/' . $sub->slug . '/' . $banner->slug) ? 'font-bold text-white bg-[#FF9700]' : '' }}">
													{{ $banner->title }}
												</a>
											</li>
											@endforeach
